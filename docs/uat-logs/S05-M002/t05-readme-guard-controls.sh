#!/usr/bin/env bash
#
# S05/T05 - negative controls for the README guard.
#
# `PackagePurityTests.Readme_documents_the_install_line_usage_and_the_shipped_surface` asserts that the
# README the package ships as its PackageReadmeFile still carries the facts a consumer copies: the
# install line built from the library project's own PackageId and Version, the consumer namespace URI
# and its `tni` prefix, the consumer sections, the two boundary sentences (coexistence and the
# activation wording rule), and all twenty public type names. A guard that has never been seen to fail
# is not a guard, so each control breaks exactly one of those facts in the real README, runs the real
# test, and records the failure message - then restores the file and proves it is byte-for-byte
# unchanged.
#
...
#
# The shell environment is repaired first, exactly as the S07 and S05 scripts do and for the same
...
#
# Usage (from the repository root): bash docs/uat-logs/S05-M002/t05-readme-guard-controls.sh
#
set -u

LOG=docs/uat-logs/S05-M002/t05-readme-guard-controls.txt
...
SCRATCH="$TEMP/t05-readme-guard-controls"
...
{
  printf 'S05/T05 - negative controls for the README guard\n'
  printf 'date:          %s\n' "$(date '+%Y-%m-%d %H:%M:%S')"
  printf 'machine:       %s\n' "$(uname -n)"
...
# --- Control 1: a toast type the README must name is no longer named anywhere --------------
# Every occurrence, not just the first: the name appears in the surface list and again in the
# failure-surface prose, and a control that rewrote one of them would leave the guard legitimately
# satisfied by the other.
control \
  'control 1 - ToastNotificationSetting is no longer named anywhere in the README' \
  'text.replace("ToastNotificationSetting", "ToastNotificationState")' \
  'does not name [ToastNotificationSetting]'

# --- Control 2: the coexistence boundary sentence is removed ------------
# The sentence is hard-wrapped in the README, so it is removed with a regex that spans the wrap. The
# guard collapses whitespace before comparing, so a reworded or removed sentence cannot pass.
control \
  'control 2 - the coexistence sentence is removed from the README' \
  're.sub(r"Toasts and balloons are independent:.*?re-routes a toast\.\s*", "", text, count=1, flags=re.S)' \
  'must carry this boundary sentence verbatim'

# --- Control 3: the new toast section heading is renamed -----------------------------------
control \
  'control 3 - the "## Toast notifications" section is renamed' \
  'text.replace("## Toast notifications", "## Toast support", 1)' \
  "must carry a '## Toast notifications' section"

# --- Control 4: the install line names a version the project does not declare --------------
control \
  'control 4 - the install line advertises version 9.9.9 (the version-bump trap)' \
  'text.replace("Version=\"1.0.0\"", "Version=\"9.9.9\"", 1)' \
  'must show the install line a consumer copies'

# --- Control 5: the consumer prefix disappears from the markup snippet ---------------------
control \
  'control 5 - the xmlns:tni prefix is removed from the README' \
  'text.replace("xmlns:tni=", "xmlns:tray=")' \
  "not the 'tni' prefix"
